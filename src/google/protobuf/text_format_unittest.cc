...
TEST_F(TextFormatParserTest, ParseInfoTreeAnyBuilding) {
  proto2_unittest::TestAny message;
  const Descriptor* d = message.GetDescriptor();

  std::string stringData =
      "any_value {\n"
      "  [type.googleapis.com/proto2_unittest.TestAny] {\n"
      "    int32_value: 1\n"
      "  }\n"
      "}";

  TextFormat::ParseInfoTree tree;
  ExpectSuccessAndTree(stringData, &message, &tree);

  EXPECT_THAT(
      tree.GetFieldLocation(d->FindFieldByName("any_value")),
      MatchesLocation(
          stringData,
          ExpectedLocation{
              .full = "any_value {\n"
...
TEST_F(TextFormatParserTest, ParseInfoTreeExtensionBuilding) {
  ...
  EXPECT_THAT(tree.GetFieldLocation(
...
TEST_F(TextFormatParserTest, ParseInfoTreeLegacyApiMatchesNewApi) {
...

  {
    ...
    ASSERT_OK_AND_ASSIGN(TextFormat::FieldLocation location_new,
                         tree.GetFieldLocation(singular_field));
